...
int _tmain(int argc, _TCHAR* argv[])
{
    ...
    const Sorter bubble_sorter(std::make_unique<BubbleSort>());
    sortVector(bubble_sorter, original, "Bubble");

    const Sorter insertion_sorter(std::make_unique<InsertionSort>());
    sortVector(insertion_sorter, original, "Insertion");

    const Sorter selection_sorter(std::make_unique<SelectionSort>());
